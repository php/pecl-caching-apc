...
/* {{{ */
void* apc_sma_protect(apc_segment_t *segment, void *p)
{
    size_t offset;
	TSRMLS_FETCH();
...
/* {{{ */
void* apc_sma_unprotect(apc_segment_t *segment, void *p)
{
    size_t offset;
    ...
    offset = (size_t)((char *)p - (char*)segment->roaddr);

    if(p >= segment->roaddr && offset < segment->size) {
        return segment->shmaddr + offset;
    }
...
#else
/* {{{ */
void* apc_sma_protect(apc_segment_t *segment, void *p) { return p; }
void* apc_sma_unprotect(apc_segment_t *segment, void *p) { return p; }
/* }}} */
#endif
